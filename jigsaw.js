...
function drawPuzzleSideShape(shaped, pg, side, curve, size, h, style) {
	...
	if (curve != NONE) {
		for (var bezier of shape) {
			pg.bezierVertex(
				...bezier.map(
					(value, i) => (curve == IN && i % 2 != 0 ? -value : value) * s
				)
			);
		}
...
function puzzleify(tiles) {
	var w = parseInt(tiles.sizeX),
		h = parseInt(tiles.sizeY);
...
function maskify(img) {
	...
	for (let i = 0; i < img.pixels.length; i += 4) {
		...
		img.pixels[i + 3] =
			(img.pixels[i + 0] + img.pixels[i + 0] + img.pixels[i + 0]) / 3;
	}
	img.updatePixels();
...
function cut(img, cols, rows, alignX, alignY) {
	...
	if (squared) {
		...
		if (ratio.y > ratio.x) sizeY *= ratio.x / ratio.y;
		else if (ratio.x > ratio.y) sizeX *= ratio.y / ratio.x;
	...
	for (var x = 0; x < cols; x++) {
		...
		for (var y = 0; y < rows; y++) {
			...
			var sliced = img.get(
				(x + 1) * sizeX + offset.x - sizeX / 2,
				(y + 1) * sizeY + offset.y - sizeY / 2,
				sizeX * 2,
				sizeY * 2
			);
